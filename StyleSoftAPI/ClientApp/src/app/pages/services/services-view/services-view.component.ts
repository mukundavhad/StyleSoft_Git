...
redirectToAddNew() {
        const ref = this.dialog.open(ServicesDetailsComponent, { data: {}, modalConfig: { title: 'Add/Edit Services Details', width: '75%',height:'80%' }, isEditable: false });
  ...
  public onUserRowSelect(event){
    //console.log(event);   //this select return only one page rows
      const ref = this.dialog.open(ServicesDetailsComponent, { data: event.data, modalConfig: { title: 'Add/Edit Services Details', width: '75%', height: '80%' }, isEditable: true });
      ref.afterClosed.subscribe(result => {
          this.RefreshGrid();
      });
  }
